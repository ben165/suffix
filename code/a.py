#!//home/asdf/temp/p3/bin/python3
from ete3 import Tree#, TreeStyle

class TrieNode:
  def __init__(self):
    # Array for child nodes of each node
    self.child = [None] * 26
    # for end of word
    self.word_end = False
  c = "" #Zusatz

class TrieNodeClean:
  child = []
  word_end = False
  end1 = 0
  c = ""

class Node1:
  def __init__(self, c, child):
    self.c = c
    self.child = child
    self.end1 = 0

class Node2:
  def __init__(self, c):
    self.c = c

    self.nodes = []
    for i in range(0, 26):
      self.nodes.append(None)

class Node3:
  def __init__(self, c, nodes):
    self.c = c
    self.nodes = nodes
    self.end1 = 0

def tree2postfix2(r, s):
  len1 = len(r.child)
  for i in range(0, len1):
    if (i == 0):
      s.append("(")
    tree2postfix2(r.child[i], s)
    if (i == len1-1):
      s.append(r.child[i].c + ")")
    else:
      s.append(r.child[i].c + ",")


def tree2newick(r, str1):
  #
  # Postfix
  # ( = beim runtergehen, linkeste node
  # , = beim rausgehen - stimmt aber nicht wenn es zum vater geht
  # ) = wenn man zum Vater geht
  #
  len1 = len(r.child)
  for i in range(0, len1):
    #print(r.child[i].c + "   len: " + str(len1))
    if len1 == 1:
      #print("hallo", end="")
      str1.append("(")
      r.child[i].end1 = 1
    elif i == 0:
      #print("(", end="")
      str1.append("(")
    elif i == len1-1:
      r.child[i].end1 = 1

    tree2newick(r.child[i], str1)
    
  
  #print(r.c + ",", end="")
  if r.end1 != 1:
    str1.append(r.c + ",")
  else:
    str1.append(r.c)
  if r.end1 == 1:
    #print(")", end="")
    str1.append(")")
    
  
  #print(r.c + ",", end="")
  if r.end1 != 1:
    str1.append(r.c + ",")
  else:
    str1.append(r.c)
  if r.end1 == 1:
    #print(")", end="")
    str1.append(")")



# LVR Inorder
def inorder(node, list1):
  if (node != None): # && node.value != null
    for i in range(0, len(node.nodes)):
      inorder(node.nodes[i], list1)
      list1.append(node.c)
    #inorder(node.l, list1)
    #list1.push(node.value)
    #inorder(node.r, list1)



# geht vermutlich nicht?
#def breitensuche(node, list1):
#  queue = []
#      
#  if (node != None):
#    queue.push(node)
#    
#  while ( queue.length != 0 ):
#    temp = queue.splice(0,1)[0]
#    list1.push( temp.value )
#  
#    if (temp.l != None):
#      queue.push(temp.l)
#    if (temp.r != None):
#      queue.push(temp.r)


if (1==2):
  #list1 = ['bear', 'bell', 'bid', 'bull', 'stock', 'stop']
  list1 = ['bear', 'bell']
  
  root = Node2("root")
  for i in range(0, len(list1)):
    for j in range(0, len(list1[i])):
      #print(list1[i][j], end="")
      nr = ord( list1[i][j] ) - 97
      if root.nodes[nr] == None:
        print(list1[i][j] + " Char not found")
  
    print()



if 1==2:
  a0 = Node1("e", [])

  a1 = Node1("mize", [])
  a2 = Node1("nimize", [])
  a3 = Node1("ze", [])

  a4 = Node1("nimize", [])
  a5 = Node1("ze", [])

  a6 = Node1("nimize", [])

  a7 = Node1("ze", [])

  a8 = Node1("i", [a1, a2, a3])
  a9 = Node1("mi", [a4, a5])

  r = Node1("", [a0, a8, a9, a6, a7])

  s = []
  tree2postfix2(r, s)
  s = ''.join(s)+";"
  print(s)

  t = Tree(s, format=1)
  print(t.get_ascii(show_internal=True))


if 1==1:
  r = Node1("", [])
  a = Node1("anana$", [])
  r.child.append(a)

  a0 = Node1("a$", [])
  a00 = Node1("$", [])
  a01 = Node1("an$", [])
  a.child.append(a0)

  a0.child.append(a00)
  a0.child.append(a01)

  a1 = Node1("nana$", [])
  a10 = Node1("na$", [])
  a11 = Node1("banana$", [])
  a1.child.append(a10)
  a1.child.append(a11)
  a.child.append(a1)

  s = []
  tree2postfix2(r, s)
  s = ''.join(s)+";"
  print(s)

  t = Tree(s, format=1)
  print(t.get_ascii(show_internal=True))

if 2==1:
  r = Node1("", [])
  a0 = Node1("a", [])
  r.nodes.append(a0)
  a1 = Node1("")


#list1 = []
#inorder(r, list1)
#print(list1)


def meins(node, list1):
  queue = []
  # Fuege erste Reihe hinzu
  for i in range(0, len(node.nodes)):
    if node.nodes[i] != None:
      queue.append( node.nodes[i] )
  # Plotte Baum
  
  # Fuege zweite Reihe hinzu
  for i in range(0, len(queue)):
    pass
    

# https://www.geeksforgeeks.org/trie-insert-and-search/

# Method to insert a key into the Trie
def insert_key(root, key):

    # Initialize the curr pointer with the root node
    curr = root

    # Iterate across the length of the string
    for c in key:
    
        # Check if the node exists for the 
        # current character in the Trie
        index = ord(c) - ord('a')
        #print(index)
        if curr.child[index] is None:
            # If node for current character does 
            # not exist then make a new node
            new_node = TrieNode()
            new_node.c = chr(ord('a') + index) # Zusatz
            # Keep the reference for the newly
            # created node
            curr.child[index] = new_node
        # Move the curr pointer to the
        # newly created node
        curr = curr.child[index]
    # Mark the end of the word
    curr.word_end = True

# Method to search a key in the Trie
def search_key(root, key):

    # Initialize the curr pointer with the root node
    curr = root

    # Iterate across the length of the string
    for c in key:

        # Check if the node exists for the 
        # current character in the Trie
        index = ord(c) - ord('a')
        if curr.child[index] is None:
            return False

        # Move the curr pointer to the 
        # already existing node for the 
        # current character
        curr = curr.child[index]

    # Return true if the word exists 
    # and is marked as ending
    return curr.word_end





if 1==2:
  # One by one search strings
  search_keys = ["do", "gee", "bat"]
  for s in search_keys:
    print(f"Key : {s}")
    if search_key(root, s):
      print("Present")
    else:
      print("Not Present")

# Print tree
#def tree2postfix(r):
#  for i in range(0, len(r.child)):
#    if r.child[i] != None:
#      print(chr(i))
#      tree2postfix(i)

# node.elm sei der jeweils gespeicherte Buchstabe
def compress(tree):
  nodes = tree.nodes
  if len(nodes) == 0:
    return tree
  elif len(nodes) ==  1:
    subtree = compress(nodes[0])
    subtree.c = nodes[0].c + subtree.c
    return subtree
  else:
    for i in range(len(nodes)):
      nodes[i] = compress(nodes[i])


def treeClean(r):
  try:
    for i in range(0, len(r.child)):
      if r.child[i] == None:
        r.child.pop(i)
      else:
        treeClean(r.child[i])
  except:
    pass

def tree2postfix(r):
  for i in range(0, len(r.child)):
    if r.child[i] != None:
      #print(r.child[i])
      #print(chr(ord('a')+ i), end=" ")
      print(r.child[i].c)
      #print(r.child[i].word_end)
      
      if r.child[i].word_end:
        print()
      
      tree2postfix(r.child[i])
#print( len(root.child) )


#def tree2newick2(r):
#  for i in range(0, len(r.child)):  
#    print("(")
#    tree2postfix2(r.child[i])
#    print(r.child[i].c, end="")


def breitensuche(node):
  queue = []
  if (node != None):
    queue.append(node)
  while ( len(queue) != 0 ):
    temp = queue.pop(0)
    print(temp.c)
    for i in range(0, len(temp.child)):
      if temp.child[i] != None:
        queue.append(temp.child[i])
      else:
        print("empty")
        #print(" ")

# tree copy?

#geht nicht.
def copyTreeClean(node: TrieNode):
  if node is None:
    return None
  newNode = TrieNodeClean()
  newNode.word_end = node.word_end
  newNode.end1 = node.end1
  newNode.c = node.c
  for i in range(0, len(node.child)):
    if not(node.child[i] is None):
      temp = copyTreeClean(node.child[i])
      if temp is None:
        pass
      else:
        newNode.child.append(temp)
  return newNode


if 1==2:
  # Create an example Trie
  root = TrieNode()
  arr = ["and", "ant", "do", "geek", "dad", "ball", "abcd", "ddb", "dda", "bbad"]
  #arr = ["aab", 'ab', 'ba']#, "abcd", "ddb", "dda", "bbad"]
  arr = ["bear", "bell", "bid", "bull", "stock", "stop"]

  for s in arr:
    insert_key(root, s)

  for i in range(0, 1000):
    treeClean(root)

  s = []
  tree2postfix2(root, s)
  s = ''.join(s)+";"
  print(s)

  t = Tree(s, format=1)
  print(t.get_ascii(show_internal=True))

  #print(treeClean(root))

  #str1 = []
  #tree2newick(root, str1)
  #tree_str = ''.join(str1)[0:-1]+";"
  #print(tree_str)

#a = TrieNode()